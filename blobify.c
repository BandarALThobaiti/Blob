...
bool blb_blob_get(blb_blob_t *blob, uint8_t *value) {
    ...
    int32_t current_offset = blob->cursor->offset;

    if (!blb_range_in(blob->range, current_offset)) {
        return false;
    }

    if (!blb_block_get(blob->block, current_offset, value)) {
        return false;
    }

    
    int32_t next_offset = current_offset + blob->range->step;
    if (blb_range_in(blob->range, next_offset)) {
        blb_cursor_step(blob->cursor, blob->range->step);
    }

    return true;
}

bool blb_blob_set(blb_blob_t *blob, uint8_t value) {
    ...
    if (!blb_block_put(blob->block, current_offset, value)) {
        return false;
    }
...
blb_blob_t *blob = blb_blob_create(3, 1); 
    
    if (blob) {
        blob->cursor->offset = 0; 

        uint8_t read_val = 0;

        blb_blob_set(blob, 0xAA); 

        blb_blob_get(blob, &read_val);
        printf("%02X\n", read_val);  
        

        blb_blob_set(blob, 0xBB); 
        blb_blob_get(blob, &read_val);
        printf("%02X\n", read_val); 
        
        
        blb_blob_set(blob, 0xCC); 
        blb_blob_get(blob, &read_val);
        printf("%02X\n", read_val);  
